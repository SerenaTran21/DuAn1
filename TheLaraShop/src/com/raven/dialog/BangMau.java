...
public class BangMau extends javax.swing.JDialog {
    ...
    void fillCobobox() {
        ...
        listMau = MauDAO.selectByMaSP(maSP);
    ...
    public void fillTableMau() {
        ...
        List<MauSac> listMau = new ArrayList<>();
        MauSacDAO MauDAO = new MauSacDAO();
        listMau = MauDAO.selectMauNotInMaSP(maSP);
        for (MauSac mau : listMau) {
    ...
    void delete() {
        ...
        if (cbxMau.getSelectedItem().equals("")) {
            ...
            if (this.showConfirm("Bạn thật sự muốn xóa?", "Xóa thành công")) {
                MauSac enity = MauDAO.selectByMaSP_Ten(maSP, (String) cbxMau.getSelectedItem());
                MauDAO.Delete(enity.getMaMau());
                fillCobobox();
                fillTableMau();
            }
        }
    ...
    private void tblBangMauMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblBangMauMouseClicked
        ...
        tenMau = (String) tblBangMau.getValueAt(row, 1);
